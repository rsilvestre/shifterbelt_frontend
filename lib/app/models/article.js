...
ArticleSchema.methods = {
  ...
  uploadAndSave: function uploadAndSave(images, cb) {
  ...
  addComment: function addComment(user, comment, cb) {
...
ArticleSchema.statics = {
  ...
  load: function load(id, cb) {
    this.findOne({ _id: id }).populate('user', 'name email username').populate('comments.user').exec(cb);
  },
  ...
  list: function list(options, cb) {
    ...
    this.find(criteria).populate('user', 'name username').sort({ 'createdAt': -1 }) // sort by date
